...
### 只创建表不需要创建模型 （例如用于创建用户和商品的关联表）
    php artisan make:migration create_xxx_xxx_xxx_table --create=xxx_xxx_xxx （这里的xxx都需要一致）
    
### 创建外键约束（当A表里的数据被删除时，关联表的数据就会一同被删除，确保数据的一致性和完整性）
    1.在database/migartions/create_xxx_xxx_table.php文件里
        $table->foreign('product_id') // 指定当前表的外键字段
            ->reference('id')         // 指定关联的表的主键字段
            ->on('product')           // 指定要关联的表名
            ->onDelete('cascade');    // 指定输出管理记录时的操作

    2.之后再模型里写下关联的方法（如果是用户和商品关联，那就再Users模型里写）
...
### route_class()
    在balde.php中使用这个route_class() 可以获取到你的路由别名并且改造成xxx-xxx-page
        例如：<div id="app" class="{{ route_class() }}-page"></div>
             Route:get('products/favorites', 'ProductsController@favorites')->name('products.favorites')
        这个class就是 products-favorites-page 在blade里写就是可以进行页面的区分
...
------------------------------------------------------------------
## 优雅的处理异常
    1.创建异常 App/Exceptions 
        php artisan make:exception InvalidRequestException
    2. 编写InvalidRequestException 文件 （用户错误行为触发的异常，比如访问下架的商品）
    ...
    3. 用户的异常行为不需要写入日志里进行提示，所以可以在app/Exceptions/Handler.php里屏蔽
        protected $dontReport = [
            InvalidRequestException::class,
        ];

    4. 创建系统内部的异常文件（比如数据库连接失败）
        这个文件就是要把数据库连接失败的问题存到日志里，但是给用户展示的就是 "系统出错了！"

        php artisan make:exception InternalException
    5. 编写系统内部异常文件
        // 创建一个给用户提示的属性 $msgForUser
        protected $msgForUser;

        public function __construct(string $message, string $msgForUser = '系统内部错误', int $code = 500)
        {
            parent::__construct($message, $code);
            // 并且把$msgForUser 用this进行指向
    ...
    备注：在异常处理文件里，$message、$code 都是可以直接用$this->message 、 $this->code进行访问到(前提要写构造函数__construct())，但是你要加属性，只能通过protected增加
        
        例如：protected $msgForUser;
        public function __construct(string $message, string $msgForUser = '系统内部错误', int $code = 500) {
            parent::__construct($message,$code)
            $this->msgForUser = $msgForUser
        }

        使用的时候就是 $this->msgForUser;
...
boolval()
    用于把值转为布尔值
    例如：$favored = boolval($user->favoriteProducts()->find($product->id));
